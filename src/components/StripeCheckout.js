...
const apiKey = "pk_test_51NaffVJ3W28RbuovVJflbJ5rIueRwaVEcJVV0m60R5VcUGMbjxUYrqs3CEttgGchCa1866ywEhf28D0MS47IS3rx00XB6huZ13"
...
const CheckoutForm = () => {
  ...
  const createPaymentIntent = async () => {
    try {
      ...
      // console.log("Here's the error: " + error.response)
    }
  };
  useEffect(() => {
    createPaymentIntent();
  ...
  return (
    <div>
      {succeeded ? (
        ...
        <article>
          <h4 className='hello'>Hello, {myUser && myUser.name}</h4>
          <p className='total-test'>Your total is {formatPrice(shipping_fee + total_amount)}</p>
          <p className='total-test'>Test Card Number: 4242 4242 4242 4242</p>
        </article>
      )}
      <form action="/create-checkout-session" id='payment-form' onSubmit={handleSubmit}>
        <CardElement
          id='card-element'
